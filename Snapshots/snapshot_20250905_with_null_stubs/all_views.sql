  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "TR2000_STAGING"."V_API_CALLS_PER_RUN
" ("RUN_ID", "RUN_TYPE", "RUN_START", "RUN_END", "RUN_STATUS", "UNIQUE_ENDPOINTS
_CALLED", "TOTAL_API_CALLS", "API_CALL_COUNT", "PROCESSING_COUNT", "TOTAL_DATA_M
B", "AVG_DURATION_MS", "TOTAL_DURATION_SECONDS", "TOTAL_RECORDS_PROCESSED", "TOT
AL_RECORDS_INSERTED") AS
  SELECT
    r.run_id,
    r.run_type,
    ...
    SUM(CASE WHEN s.stat_type = 'PROCESSING' THEN 1 ELSE 0 END) as processing_co
unt,
...
ORDER BY r.run_id DESC;
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "TR2000_STAGING"."V_API_CALL_STATISTI
CS" ("ENDPOINT_KEY", "TOTAL_CALLS", "AVG_RESPONSE_TIME_MS", "MIN_RESPONSE_TIME_M
S", "MAX_RESPONSE_TIME_MS", "TOTAL_DATA_MB", "AVG_RESPONSE_KB", "SUCCESSFUL_CALL
S", "FAILED_CALLS", "LAST_CALL_TIME") AS
  SELECT
    endpoint_key,
    ...
    SUM(CASE WHEN api_status_code BETWEEN 200 AND 299 THEN 1 ELSE 0 END) as succ
essful_calls,
...
ORDER BY total_calls DESC;
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "TR2000_STAGING"."V_API_OPTIMIZATION_
CANDIDATES" ("ENDPOINT_KEY", "CALL_COUNT", "RUN_COUNT", "AVG_CALLS_PER_RUN", "AV
G_DURATION_MS", "TOTAL_TIME_SECONDS", "PCT_OF_TOTAL_TIME", "VOLUME_CATEGORY", "O
PTIMIZATION_PRIORITY") AS
...
SELECT
    ...
    ROUND(total_duration_ms * 100.0 / SUM(total_duration_ms) OVER(), 2) as pct_o
f_total_time,
    CASE
...
ORDER BY call_count DESC;
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "TR2000_STAGING"."V_ENDPOINT_TABLE_ST
ATISTICS" ("ENDPOINT_KEY", "TARGET_TABLE", "TOTAL_CALLS", "RUNS_WITH_THIS_ENDPOI
NT", "AVG_CALLS_PER_RUN", "TOTAL_RECORDS_PROCESSED", "TOTAL_RECORDS_INSERTED", "
AVG_DURATION_MS", "MAX_DURATION_MS", "TOTAL_DATA_MB", "LAST_CALLED") AS
  SELECT
    endpoint_key,
    CASE
	...
	WHEN endpoint_key = 'PIPE_ELEMENT_REFERENCES' THEN 'PIPE_ELEMENT_REFEREN
CES'
...
ORDER BY total_calls DESC;
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "TR2000_STAGING"."V_ETL_RUN_SUMMARY"
("RUN_ID", "RUN_TYPE", "START_TIME", "END_TIME", "RUN_DURATION_SECONDS", "STATUS
", "UNIQUE_API_ENDPOINTS", "TOTAL_API_CALLS", "TOTAL_DATA_MB", "TOTAL_RECORDS",
"ERROR_COUNT") AS
  SELECT
    r.run_id,
    r.run_type,
    ...
    ROUND(EXTRACT(MINUTE FROM (r.end_time - r.start_time)) * 60 +
	  EXTRACT(SECOND FROM (r.end_time - r.start_time)), 2) as run_duration_s
econds,
    r.status,
    (SELECT COUNT(DISTINCT endpoint_key) FROM ETL_STATISTICS WHERE run_id = r.ru
n_id AND stat_type = 'API_CALL') as unique_api_endpoints,
    (SELECT COUNT(*) FROM ETL_STATISTICS WHERE run_id = r.run_id AND stat_type =
 'API_CALL') as total_api_calls,
    (SELECT SUM(api_response_size)/1024/1024 FROM ETL_STATISTICS WHERE run_id =
r.run_id) as total_data_mb,
    (SELECT SUM(records_processed) FROM ETL_STATISTICS WHERE run_id = r.run_id)
as total_records,
    (SELECT COUNT(*) FROM ETL_ERROR_LOG WHERE error_timestamp BETWEEN r.start_ti
me AND NVL(r.end_time, SYSTIMESTAMP)) as error_count
FROM ETL_RUN_LOG r
ORDER BY r.run_id DESC;


  CREATE OR REPLACE FORCE EDITIONABLE VIEW "TR2000_STAGING"."V_ETL_STATISTICS_SU
MMARY" ("RUN_ID", "ENDPOINT_KEY", "OPERATION_COUNT", "TOTAL_RECORDS_PROCESSED",
"TOTAL_RECORDS_INSERTED", "TOTAL_RECORDS_FAILED", "AVG_DURATION_MS", "MIN_DURATI
ON_MS", "MAX_DURATION_MS", "TOTAL_DURATION_MS", "FIRST_OPERATION", "LAST_OPERATI
ON", "SUCCESS_COUNT", "FAILURE_COUNT", "SUCCESS_RATE_PCT") AS
  SELECT
    run_id,
    endpoint_key,
    ...
    ROUND(SUM(CASE WHEN status = 'SUCCESS' THEN 1 ELSE 0 END) * 100.0 / COUNT(*)
, 2) as success_rate_pct
FROM ETL_STATISTICS
GROUP BY run_id, endpoint_key;


  CREATE OR REPLACE FORCE EDITIONABLE VIEW "TR2000_STAGING"."V_OPERATION_STATIST
ICS" ("OPERATION_NAME", "EXECUTION_COUNT", "AVG_DURATION_MS", "MIN_DURATION_MS",
 "MAX_DURATION_MS", "TOTAL_RECORDS_PROCESSED", "TOTAL_RECORDS_INSERTED", "TOTAL_
RECORDS_UPDATED", "TOTAL_RECORDS_DELETED", "TOTAL_RECORDS_FAILED", "SUCCESS_COUN
T", "FAILURE_COUNT", "SUCCESS_RATE") AS
  SELECT
    operation_name,
...
ORDER BY execution_count DESC;
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "TR2000_STAGING"."V_RAW_JSON" ("RAW_J
SON_ID", "ENDPOINT_KEY", "ENDPOINT_TEMPLATE", "ENDPOINT_VALUE", "PLANT_ID", "ISS
UE_REVISION", "PCS_NAME", "PCS_REVISION", "ENDPOINT_CATEGORY", "DATA_STATUS", "B
ATCH_ID", "API_CALL_TIMESTAMP", "CREATED_DATE", "PAYLOAD_SIZE", "PAYLOAD_PREVIEW
") AS
  SELECT
    raw_json_id,
    endpoint_key,
    ...
    CASE
	WHEN endpoint_value LIKE '%/pcs/%/rev/%' THEN
	    REGEXP_SUBSTR(endpoint_value, '/pcs/[^/]+/rev/([^/]+)', 1, 1, NULL,
1)
	ELSE NULL
    END AS pcs_revision,
	...
	WHEN endpoint_key LIKE 'PCS_%' AND endpoint_key != 'PCS_LIST' THEN 'PCS
Details'
	...
	WHEN DBMS_LOB.SUBSTR(payload, 2, 1) = '[]' OR DBMS_LOB.SUBSTR(payload, 2
, 1) = '{}' THEN 'Empty Result'
	ELSE 'Has Data'
    END AS data_status,
    batch_id,
	...
	WHEN DBMS_LOB.GETLENGTH(payload) < 50 THEN TO_CHAR(DBMS_LOB.SUBSTR(paylo
ad, 50, 1))
...
FROM RAW_JSON;
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "TR2000_STAGING"."V_RAW_JSON_SUMMARY"
 ("ENDPOINT_KEY", "ENDPOINT_CATEGORY", "CALL_COUNT", "BATCH_COUNT", "PLANT_COUNT
", "ISSUE_COUNT", "PCS_COUNT", "SUCCESSFUL_CALLS", "FAILED_CALLS", "MIN_PAYLOAD_
SIZE", "MAX_PAYLOAD_SIZE", "AVG_PAYLOAD_SIZE", "TOTAL_PAYLOAD_SIZE", "FIRST_CALL
", "LAST_CALL") AS
  WITH raw_data AS (
    SELECT
	endpoint_key,
	CASE
	    WHEN endpoint_key LIKE '%_REFERENCES' THEN 'Reference Data'
	    WHEN endpoint_key LIKE 'PCS_%' AND endpoint_key != 'PCS_LIST' THEN '
PCS Details'
	    WHEN endpoint_key = 'PCS_LIST' THEN 'PCS List'
	    WHEN endpoint_key = 'VDS_CATALOG' THEN 'VDS Catalog'
	    ELSE 'Other'
	...
	CASE
	    WHEN endpoint_value LIKE '%/issues/rev/%' THEN
		REGEXP_SUBSTR(endpoint_value, '/issues/rev/([^/]+)', 1, 1, NULL,
 1)
	    ELSE NULL
	END AS issue_revision,
	CASE
...
SELECT
    ...
    SUM(CASE WHEN data_status = 'Has Data' THEN 1 ELSE 0 END) as successful_call
s,
